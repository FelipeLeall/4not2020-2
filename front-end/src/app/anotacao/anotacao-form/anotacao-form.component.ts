...
export class AnotacaoFormComponent implements OnInit {
  ...
  constructor(
    private turmaSrv : AnotacaoService,
  ...
  async ngOnInit() {
    ...
    if(this.actRoute.snapshot.params['id']) {
      try {
        ...
        this.anotacao = await this.turmaSrv.obterUm(this.actRoute.snapshot.params['id'])
    ...
    if(form.valid) {
      try {
        ...
        if(this.anotacao._id) {
          await this.turmaSrv.atualizar(this.anotacao) // Atualização
        }
        else {
          await this.turmaSrv.novo(this.anotacao)
        }
